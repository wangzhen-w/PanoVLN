...
def discover_scenes(
    ...
    # Keep train/val asset layouts working; also accept flattened scene folders.
    folders = {
        path.parent
        for pattern in ("*/*.basis.glb", "*/*/*.basis.glb")
...
def resolve_scene_path(scene_root: str | Path, scene_id: str) -> Path:
    ...
    if len(parts) not in (2, 3) or path.is_absolute() or ".." in parts:
        raise ValueError(f"Invalid portable HM3D scene id: {scene_id}")
    if len(parts) == 3 and parts[0] not in {"train", "val", "test"}:
        raise ValueError(f"Invalid HM3D asset split in scene id: {scene_id}")
    root = Path(scene_root).resolve()
    result = (root / Path(*parts)).resolve()
    result.relative_to(root)
    if result.is_file():
        return result
    # Existing annotations keep their split prefix after assets are flattened.
    if len(parts) == 3:
        flat_result = (root / Path(*parts[1:])).resolve()
        flat_result.relative_to(root)
        if flat_result.is_file():
            return flat_result
    else:
        # Some public annotations (e.g. ScaleVLN) omit the asset split.
        matches = []
        for split in ("train", "val", "test"):
            candidate = (root / split / Path(*parts)).resolve()
        ...
        if len(matches) == 1:
            return matches[0]
        if len(matches) > 1:
            raise ValueError(f"Ambiguous HM3D scene id across asset splits: {scene_id}")
