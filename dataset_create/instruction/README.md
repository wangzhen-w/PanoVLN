...
依赖 Habitat-Sim 0.3.3、NumPy、SciPy、NetworkX、Pillow、requests、imageio 和 imageio-ffmpeg。先激活安装了这些依赖的 Python 环境；Qwen 通过单独部署的 vLLM 服务调用。
...
| `ERP_ROOT` | 训练图片目录，默认 `./data/images/panovln` |
...
模型回复截断、无法解析为 JSON，或反复缺少必需字段，在现有重试次数耗尽后将该轨迹标记为 `quarantined`。字段检查失败的 JSON 回复会移出可复用缓存，保留为 `.invalid.json` 供调试，避免续跑反复读取同一份无效回复。网络、HTTP 服务和程序错误仍保留为 `error`，等待恢复。汇总阶段显示 `Checking records/ERP` 进度，检查接受记录和 clean ERP 后写出 JSON；只有 `accepted` 会进入训练数据。写出并压缩 JSON、删除临时材料分别提示状态和耗时。清理期间训练文件已可用；历史备份中的大量小文件可能使清理较慢。
...
场景路径相对 `SCENE_ROOT`，本机为 `train/...` 或 `val/...`，仅用于定位 HM3D 文件；这些场景全部进入同一份训练输入。Habitat 的 `data_path` 指向导出的 `.json.gz`，`scenes_dir` 保持 `./data/scene/hm3d`。原始轨迹文件继续作为动作与图片对齐依据，不向标准 R2R 添加制作过程字段。
