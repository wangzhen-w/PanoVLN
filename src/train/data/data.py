...
VLN_SYSTEM_PROMPT = (
    ...
    "Return exactly 18 action words in execution order, separated by spaces. "
    "If the task is complete before 18 actions, "
    "fill the remaining positions with stop."
)
...
def _extract_real_action_count(
    ...
    if value < 1 or value > VLN_ACTION_SEQUENCE_LENGTH:
        raise ValueError(
            ...
            f"[1, {VLN_ACTION_SEQUENCE_LENGTH}], "
            f"got {value}"
        )
