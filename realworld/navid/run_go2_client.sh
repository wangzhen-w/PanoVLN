...
for arg in "$@"; do
    if [[ "$arg" == "--help" || "$arg" == "-h" || "$arg" == "--print-config" ]]; then
        exec "$PYTHON_BIN" -m realworld.navid.go2_client "${CLIENT_ARGS[@]}"
    fi
done

CONTROL_BACKEND="$("$PYTHON_BIN" -c 'from realworld.navid.go2_client import parse_args; print(parse_args().control_backend)' "${CLIENT_ARGS[@]}")"
