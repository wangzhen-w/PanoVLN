...
import torch
from transformers import PretrainedConfig, SiglipImageProcessorPil, SiglipVisionModel
...
class SiglipVisionTower(VisionTower):
    def __init__(self, model_name_or_path: str, config: PretrainedConfig, state_dict=None):
        ...
        self.image_processor = SiglipImageProcessorPil.from_pretrained(model_name_or_path)
...
class SiglipVisionTowerS2(VisionTowerS2):
    def __init__(self, model_name_or_path: str, config: PretrainedConfig):
        super().__init__(model_name_or_path, config)
        self.image_processor = SiglipImageProcessorPil.from_pretrained(model_name_or_path)
        self.vision_tower = SiglipVisionModel.from_pretrained(model_name_or_path, dtype=eval(config.model_dtype))
