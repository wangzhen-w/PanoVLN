# coding=utf-8
# JanusVLN checkpoint-compatible runtime adapted for Transformers 5.5.
...
from .vggt.models.aggregator import Aggregator, _RESNET_MEAN, _RESNET_STD
...
class Qwen2_5_VLPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        # Transformers 5 initializes missing tensors after loading. Its init
        # helpers preserve loaded parameters; raw .data writes overwrite them.
        super()._init_weights(module)
        if isinstance(module, Qwen2_5_VisionRotaryEmbedding):
            inv_freq = 1.0 / (
                module.theta ** (torch.arange(0, module.dim, 2, dtype=torch.float32) / module.dim)
            )
            init.copy_(module.inv_freq, inv_freq)
        elif isinstance(module, Aggregator):
            # These non-persistent buffers are absent from saved checkpoints.
            init.copy_(module._resnet_mean, torch.tensor(_RESNET_MEAN).reshape(1, 1, 3, 1, 1))
            init.copy_(module._resnet_std, torch.tensor(_RESNET_STD).reshape(1, 1, 3, 1, 1))
...
class Qwen2_5_VLRotaryEmbedding(nn.Module):
    def __init__(self, config: Qwen2_5_VLConfig, device=None):
        ...
        self.config = config
        self.rope_init_fn = (
            self.compute_default_rope_parameters if self.rope_type == "default"
            else ROPE_INIT_FUNCTIONS[self.rope_type]
        )

        inv_freq, self.attention_scaling = self.rope_init_fn(self.config, device)
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        self.register_buffer("original_inv_freq", self.inv_freq.clone(), persistent=False)

    @staticmethod
    def compute_default_rope_parameters(config, device=None, seq_len=None):
        dim = config.hidden_size // config.num_attention_heads
        inv_freq = 1.0 / (
            config.rope_theta ** (torch.arange(0, dim, 2, device=device, dtype=torch.float32) / dim)
        )
        return inv_freq, 1.0
...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._flash_attn_uses_top_left_mask = not is_flash_attn_greater_or_equal(library_version="2.1.0")
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        using_sliding_window_cache = (isinstance(past_key_values, StaticCache) and any(past_key_values.is_sliding))
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if config.sliding_window is not None:
                ...
                if not (isinstance(past_key_values, StaticCache) and any(past_key_values.is_sliding)) or sequence_length > target_length:
...
class Qwen2_5_VLForConditionalGenerationForJanusVLN(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, mode=None, **kwargs):
        ...
        model = loaded[0] if kwargs.get("output_loading_info") else loaded
        model.mode = mode
        if vggt_model_path:
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Janus computes multimodal RoPE in forward. Derive the absolute cache
        # positions from the sliced input, since generate() no longer supplies them.
