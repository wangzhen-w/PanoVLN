...
from realworld.panovln.client_config import config_arguments
from realworld.panovln.navigation_metrics import NavigationMetrics, build_navigation_reports
...
# Zero executes the complete 18-action sequence advertised by the server.
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--method-name", default="PanoVLN", help="Method label for experiment aggregation.")
    ...
    try:
        ...
        if args.actions_per_replan == "uncertainty":
            if ready.get("supports_action_uncertainty") is not True:
                raise ValueError("Model server lacks action uncertainty support; update and restart realworld.panovln.server")
