...
from src.vln_config import VLN_ACTION_SEQUENCE_LENGTH, configure_vln_model
...
def parse_action_sequence(text: str) -> list[str]:
    ...
    for start, end, action_name in matches:
        ...
        last_end = end
        if len(actions) >= VLN_ACTION_SEQUENCE_LENGTH:
            break
...
def build_executable_action_queue(
    ...
    action_list = list(actions)[:VLN_ACTION_SEQUENCE_LENGTH]
    if not action_list:
        return ["stop"]
...
class PanoVLNPredictor:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            configure_vln_model(model_config)
            _log_stage(
                "model config loaded "
            ...
            _log_stage(
                ...
                f"action_sequence_length={VLN_ACTION_SEQUENCE_LENGTH} "
                "view_mode=panorama "
                f"in {_format_elapsed(step_start)}"
            )
    ...
    def predict(
        ...
        *,
        ...
        if include_uncertainty:
            uncertainty_max_actions = validate_action_count(uncertainty_max_actions)
        ...
        generation_kwargs["max_new_tokens"] = max(
            ...
            VLN_ACTION_SEQUENCE_LENGTH * 2 + 4,
        )
        _log_stage(
            "generation started "
        ...
        # Exclude preprocessing/H2D copies; include completion of GPU generation.
        inference_device = self._input_device()
