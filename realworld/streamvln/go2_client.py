...
from realworld.streamvln.client_config import config_arguments
from realworld.streamvln.camera_projection import (
    PerspectiveProjector, DEFAULT_PERSPECTIVE_HFOV,
    DEFAULT_PERSPECTIVE_WIDTH, DEFAULT_PERSPECTIVE_HEIGHT,
)
from realworld.streamvln.navigation_metrics import NavigationMetrics, build_navigation_reports
from realworld.streamvln.action_policy import (
    ATOMIC_ACTION_NAMES,
...
DEFAULT_UPLOAD_IMAGE_MODE = "raw"
DEFAULT_UPLOAD_IMAGE_SIZE = (DEFAULT_PERSPECTIVE_WIDTH, DEFAULT_PERSPECTIVE_HEIGHT)
# Zero executes the complete baseline action queue advertised by the server.
...
def write_navigation_reports(path: Path, run_log: dict, measurements: dict) -> None:
    details, summary = build_navigation_reports(run_log, measurements)
    write_navigation_json(path, details)
    write_navigation_json(path.with_name("summary.json"), summary)
        ...
        if actual_width > 0 and actual_height > 0:
            ...
            if abs(aspect - 2.0) > 0.35:
                print(
                    ...
                    "ERP input expects a stitched panoramic image close to 2:1; check the camera capture mode.",
                    file=sys.stderr,
                    flush=True,
                )
...
class PerspectiveCamera(OpenCVCamera):
    """Keep the original camera acquisition; project every consumer's frame."""
    def __init__(self, *args, projector: PerspectiveProjector, **kwargs):
        self.projector = projector
        super().__init__(*args, **kwargs)

    def read_frame(self, flush_frames: int = 0):
        return self.projector(super().read_frame(flush_frames=flush_frames))


def configured_projector(args):
    projector = PerspectiveProjector(
        args.perspective_width, args.perspective_height, args.perspective_hfov,
        args.perspective_yaw, args.perspective_pitch,
    )
    if args.upload_image_mode != "raw":
        raise ValueError("Perspective baselines require upload_image_mode=raw; frames are already projected on the client")
    if (args.video_width is None) != (args.video_height is None):
        raise ValueError("video_width and video_height must both be set or both be empty")
    if args.video_width is not None:
        if min(args.video_width, args.video_height) <= 0 or (
            args.video_width * projector.height != args.video_height * projector.width
        ):
            raise ValueError("Recorded video must have the same aspect ratio as the projected perspective view")
    return projector
...
class Ros2SportBackend(RobotBackend):
    ...
    def __init__(self, node_name: str = "pano_vln_go2_client", *, odometry_observer=None):
        import rclpy
        ...
        self._odometry_observer = odometry_observer
        # Keep ROS alive until our shutdown path has published stop. Foxy has
        # no SignalHandlerOptions; its SIGINT handler chains to Python's handler.
        try:
    ...
    def _handle_sport_mode_state(self, msg: object) -> None:
        try:
            ...
            if self._odometry_observer is not None:
                self._odometry_observer(self._odom)
        except Exception as exc:
    ...
    def close(self) -> None:
        try:
            # Send immediately, then repeat before destroying the publisher.
            # Reuse the proven zero-velocity stop command, with no new motion API.
...
class VLNHttpClient:
    def __init__(self, server_base_url: str, timeout_s: float, predict_path: str = DEFAULT_PREDICT_PATH, *,
                 uncertainty_max_actions: Optional[int] = None, metrics: Optional[NavigationMetrics] = None):
        import requests
    ...
    def predict(
        ...
        *,
        instruction: str,
        images: Sequence[bytes],
        request_source: str = "sync",
    ) -> dict:
        files = [
            ("images", (f"frame_{index:03d}.jpg", image, "image/jpeg"))
            for index, image in enumerate(images)
        ]
        data = {"instruction": instruction, "episode_id": self.episode_id,
                "observation_start": str(self.observations_sent)}
        ...
        request_id = self.metrics.begin_request(
            source=request_source, image_count=len(images), upload_bytes=sum(map(len, images)),
            server_request_id=server_request_id,
        ) if self.metrics is not None else None
        try:
            response = self.session.post(
                self.predict_url, data=data, files=files, timeout=self.timeout_s,
            ...
            if self.metrics is not None:
                self.metrics.end_request(request_id, error=exc)
            raise
        if self.metrics is not None:
            self.metrics.end_request(
                request_id, server_latency_s=result.get("latency_s"),
                server_s=result.get("server_s"), inference_s=result.get("inference_s"),
            )
        self.observations_sent += len(images)
        result["_client_request_id"] = request_id
        return result
...
def request_prediction(
    ...
    *,
    ...
    # All atom observations reach the server exactly once. Model-specific history
    # sampling happens there, before the official visual preprocessing.
    selected_images = list(history_snapshot)[client.observations_sent:]
    if not selected_images:
        raise ValueError("No new observations; do not repeat a stateful prediction")
    ...
    return PredictionResult(
        ...
        request_id=result.get("_client_request_id"),
    )
...
def build_backend(args: argparse.Namespace, *, odometry_observer=None) -> RobotBackend:
    if args.control_backend == "dry-run":
        return DryRunBackend()
    if args.control_backend == "ros2":
        return Ros2SportBackend(odometry_observer=odometry_observer)
    raise ValueError(f"Unsupported backend: {args.control_backend}")


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run StreamVLN on a Unitree Go2.")
    ...
    parser.add_argument("--method-name", default="StreamVLN", help="Method label for experiment aggregation.")
    parser.add_argument("--scene-name", default=None, help="Scene/environment label for this trial.")
    parser.add_argument("--route-id", default=None, help="Shared route ID when comparing methods.")
    parser.add_argument("--trial-id", default=None, help="Repeat/trial ID for this route.")
    ...
    parser.add_argument("--perspective-hfov", type=float, default=DEFAULT_PERSPECTIVE_HFOV)
    parser.add_argument("--perspective-width", type=int, default=DEFAULT_PERSPECTIVE_WIDTH)
    parser.add_argument("--perspective-height", type=int, default=DEFAULT_PERSPECTIVE_HEIGHT)
    parser.add_argument("--perspective-yaw", type=float, default=0.0, help="ERP centre is forward; positive yaw looks right")
    parser.add_argument("--perspective-pitch", type=float, default=0.0, help="Positive pitch looks up")
    parser.add_argument(
        "--save-output-dir",
        default="",
    ...
    parser.add_argument(
        ...
        help="Comma-separated outputs: images, video, json (navigation.json + summary.json), all, or none.",
    )
...
def build_run_id(method_name: str, scene_name: Optional[str],
                 route_id: Optional[str], trial_id: Optional[str]) -> str:
    """Use experiment labels as a single safe directory name."""
    parts = []
    for value in (method_name, scene_name, route_id, trial_id):
        part = re.sub(r"[^\w.-]+", "_", str(value).strip()) if value is not None else "null"
        parts.append(part.strip("._") or "null")
    return "_".join(parts)
...
@contextmanager
def navigation_signal_handlers():
    """Interrupt navigation without letting repeated signals skip stop/cleanup."""
    ...
    def interrupt(signum, frame):
        ...
        # Unwind spin_once/camera/network calls before publishing stop, avoiding
        # a reentrant ROS spin inside the signal handler itself.
...
def main() -> int:
    with navigation_signal_handlers():
        try:
            run_navigation()
        except KeyboardInterrupt:
            print("Interrupted; navigation exited.", flush=True)
            return 130
    return 0
...
def run_navigation() -> None:
    ...
    if args.prefetch_after_actions != 0:
        raise ValueError("Stateful baseline inference requires prefetch_after_actions=0")
    if args.actions_per_replan != 0:
        raise ValueError("Use actions_per_replan=0 to execute the official action queue")
    if abs(args.forward_distance - 0.25) > 1e-6 or abs(args.turn_degrees - 15) > 1e-6:
        raise ValueError("Baseline action conversion requires 0.25 m / 15 degree atoms")
    projector = configured_projector(args)
    ...
    run_id = build_run_id(args.method_name, args.scene_name, args.route_id, args.trial_id)
    run_dir = None
    if save_output_dir is not None:
        if requested_save_contents:
            run_dir = save_output_dir / run_id
            try:
                run_dir.mkdir(parents=True, exist_ok=False)
            except FileExistsError as exc:
                raise ValueError(
                    f"Run directory already exists: {run_dir}. "
                    "Set a different experiment.trial_id (or --trial-id) for a new run."
                ) from exc
        save_image_dir = run_dir / "images" if "images" in requested_save_contents else None
        save_video_path = run_dir / "navigation.mp4" if "video" in requested_save_contents else None
        navigation_json_path = (
            run_dir / "navigation.json"
    ...
    camera = backend = video_recorder = None
    history: deque[bytes] = deque()
    run_log: dict[str, object] = {
        "schema_version": 2,
        "run_id": run_id,
        "experiment": {
            "method_name": args.method_name, "scene_name": args.scene_name,
            "route_id": args.route_id, "trial_id": args.trial_id,
        },
        ...
        "annotations": {
            ...
            "success_distance_threshold_m": 3.0,
        ...
        "camera": {
            ...
            "output_view": "perspective",
            "projection": {
                "width": projector.width, "height": projector.height,
                "hfov": projector.hfov, "yaw": projector.yaw, "pitch": projector.pitch,
            },
        },
        "output": {
    ...
    try:
        if navigation_json_path is not None:
            write_navigation_reports(navigation_json_path, run_log, metrics.snapshot())
        client = VLNHttpClient(
            args.server_base_url, timeout_s=args.request_timeout, metrics=metrics,
            uncertainty_max_actions=args.replan_action_range[1] if args.actions_per_replan == "uncertainty" else None,
        )
        ready = client.ready()
        if ready.get("method") != args.method_name:
            raise ValueError("Connected server method differs from experiment.method_name")
        projector.validate_server(ready)
        ...
        backend = build_backend(args, odometry_observer=metrics.record_odometry)
        video_recorder = (
        ...
        reset_response = client.session.post(
            client._join_url(client.server_base_url, "/reset"),
            json={"episode_id": client.episode_id}, timeout=client.timeout_s)
        ...
        if args.actions_per_replan == "uncertainty":
            if ready.get("supports_action_uncertainty") is not True:
                raise ValueError("Model server lacks action uncertainty support; update and restart realworld.streamvln.server")
        ...
        if video_recorder is not None:
            run_log["video_started_t_s"] = metrics.now()
            video_recorder.start()
        first_frame = camera.read_jpeg(flush_frames=args.capture_flush_frames)
        run_log["observations"].append({"frame_index": 0, "t_s": metrics.now(), "label": "initial"})
        ...
        while args.max_replans <= 0 or replans < args.max_replans:
            if pending_prediction is not None:
                if not pending_prediction.done():
                    metrics.begin_wait()
            ...
            metrics.end_wait(prediction.request_id)
            replans += 1
            ...
            for action, repeat_count in action_groups:
                ...
                action_log = {
                    "replan": replans, "action_index": action_index + 1,
                    "action_count": repeat_count, "action_end_index": completed_actions,
                    "action": action, "status": "running", "stopped": False,
                    "completed_atom_count": 0, "captured_atom_frames": 0,
                    "started_t_s": None, "ended_t_s": None, "duration_s": None,
                }
                ...
                def capture_atom_frame(atom_index: int) -> None:
                    ...
                    action_log["completed_atom_count"] = max(action_log["completed_atom_count"], atom_index)
                    captured_frame = camera.read_jpeg(flush_frames=args.capture_flush_frames)
                    action_log["captured_atom_frames"] += 1
                    run_log["observations"].append({
                        "frame_index": frame_index, "t_s": metrics.now(), "replan": replans,
                        "action_index": action_index + atom_index, "action": action,
                    })
                ...
                action_log["started_t_s"] = metrics.now()
                run_log["executed_actions"].append(action_log)
                motion_interval = metrics.begin_motion() if action != "stop" else None
                try:
                    should_stop = execute_action(
                        backend, action, motion, repeat_count=repeat_count, on_atom_complete=capture_atom_frame,
                    )
                    action_log.update(status="completed", stopped=should_stop, completed_atom_count=repeat_count)
                except BaseException as exc:
                    action_log["status"] = "interrupted" if isinstance(exc, KeyboardInterrupt) else "failed"
                    action_log["error"] = repr(exc)
                    raise
                finally:
                    action_log["ended_t_s"] = metrics.now()
                    action_log["duration_s"] = time.perf_counter() - action_started_s
                    if motion_interval is not None:
                        metrics.end_motion(motion_interval)
                if should_stop:
                    break
            ...
            if should_stop:
                run_log["termination_reason"] = "model_stop"
                break
        if run_log["termination_reason"] is None:
            run_log["termination_reason"] = "max_replans"
    except KeyboardInterrupt:
        run_log["status"] = "interrupted"
        run_log["termination_reason"] = "user_interrupt"
        raise
    except Exception as exc:
        run_log["status"] = "failed"
        run_log["termination_reason"] = "exception"
        run_log["error"] = repr(exc)
        raise
    finally:
        # Once cleanup starts, even the first Ctrl+C must not interrupt stop.
        for sig in (signal.SIGINT, signal.SIGTERM):
            signal.signal(sig, signal.SIG_IGN)
        # Capture the endpoint now, but defer metric aggregation until AFTER stop.
        trial_ended_t_s = metrics.now()
        ...
        try:
            ...
            metrics.finish(trial_ended_t_s)
            run_log["efficiency"] = metrics.snapshot()
            try:
                if video_recorder is not None:
                ...
                try:
                    if navigation_json_path is not None:
                        write_navigation_reports(navigation_json_path, run_log, run_log["efficiency"])
